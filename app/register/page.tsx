...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      //prepare url address
      const url = `${process.env.NEXT_PUBLIC_API_URL}/admins`;
      //send request
  ...
  return (
    <div className="w-full h-dvh flex bg-blue-300 justify-center items-center">
      <div className="bg-slate-100 text-gray-700 w-full md:w-1/2 lg:w-1/3 p-5">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button 
          type="submit"
            className="w-full p-5 border rounded-md text-center text-white font-bold bg-blue-500 cursor-pointer hover:bg-blue-600"
            >
                register
            </button>
          <div className="flex justify-center items-center mt-2"></div>
          <a href="/register" className="text-blue-500 underline p-2">have an account</a>
        </form>
      </div>
    </div>
  );
